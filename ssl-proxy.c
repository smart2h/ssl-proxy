	...
	if (what & (BEV_EVENT_EOF | BEV_EVENT_ERROR)) {
		...
		if (partner) {
			...
			if (evbuffer_get_length(bufferevent_get_output(partner))) {
				 ...
				 * side. */
				bufferevent_setcb(partner, discard_readcb,
			...
			/* We're in drain-and-close mode (partner already gone)
			 * and the remote has half-closed.  Since it can still
			 * receive, keep writing; close_on_finished_writecb will
			 * free us when the output buffer is empty. */
			return;
		}
		bufferevent_free(bev);
	}
...
syntax(void)
{
	fputs("Syntax:\n", stderr);
	fprintf(stderr,
		"   %s [-server -cert certificate_chain_file -key private_key_file] "
		"<listen-on-addr> <connect-to-addr>\n",
		program_name);
	fputs("Example:\n", stderr);
	fprintf(stderr,
		"   %s -server -cert certificate_chain_file -key private_key_file "
		"0.0.0.0:8443 127.0.0.1:8080\n",
		program_name);
	...
	struct sockaddr *a, int slen, void *p)
{
	 ...
	 * new connection */
	if (server_mode) {
		b_in = bufferevent_openssl_socket_new(base, fd, ssl,
...
#define TLS_method SSLv23_method
	...
	if (server_mode) {
		if (!SSL_CTX_use_certificate_chain_file(
				ssl_ctx, certificate_chain_file) ||
			!SSL_CTX_use_PrivateKey_file(
				ssl_ctx, private_key_file, SSL_FILETYPE_PEM)) {
			fprintf(stderr, "Failed to read %s or %s.\n",
...
main(int argc, char **argv)
{
	...
	if (server_mode) {
		if (!certificate_chain_file || !private_key_file) {
			fputs("Both certificate_chain_file and private_key_file must be "
				  "specified when in server mode.\n",
				stderr);
			return 1;
		}
	}
